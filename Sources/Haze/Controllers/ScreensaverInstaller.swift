...
/// Installs the bundled `HazeSaver.saver` into `~/Library/Screen Savers` and
...
enum ScreensaverInstaller {
    static var bundledSaverURL: URL? {
        Bundle.main.url(forResource: "HazeSaver", withExtension: "saver")
    }

    static var installURL: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Screen Savers/HazeSaver.saver", isDirectory: true)
    }
    ...
    static func install() -> Bool {
        guard let source = bundledSaverURL else {
            Log.app.error("Bundled HazeSaver.saver not found in app bundle")
            return false
        }
