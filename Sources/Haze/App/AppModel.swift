...
final class AppModel: ObservableObject {
    ...
    func bootstrap() {
        ...
        // Once the window server settles (it isn't ready right at launch), bind the
        // wallpaper window to every Space. `reaffirm` forces it onto all managed
        // Spaces via the Spaces API — no rebuild, no flash. Ongoing binding to
        // Spaces created later (full-screen apps) is handled on each Space change.
    ...
    /// Re-bind the wallpaper window to every Space (forces it onto all managed
    /// Spaces, incl. full-screen ones) without a rebuild. Used on launch + on open.
    func reaffirmWallpaper() { wallpaper.reaffirm() }
    ...
    /// Space-switch swipes / Mission Control show a still gradient, not black), and
    /// — when the user opts in — also set as the macOS desktop picture (so the
    /// Spaces switcher / lock / login match too). The user's original is captured
    ...
    func updateSettings(_ newSettings: AppSettings) {
        ...
        if matchChanged {
            if newSettings.matchSystemWallpaper {
                ...
                syncSystemWallpaper()   // setSystem=false → keep the in-window backing poster; system picture stays restored
            }
        }
    }
